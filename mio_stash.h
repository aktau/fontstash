/*
 * Copyright (c) 2013 Tor Andersson. All rights reserved.
 *
...
struct font *load_font_from_memory(const char *filename, unsigned char *data, int len) {
    ...
    if (!ok) {
        printf("error: cannot init font file: '%s'", filename);
        return NULL;
    }
...
struct font *load_font(const char *filename) {
    ...
    if (!font->data) {
        printf("error: cannot load font file: '%s'", filename);
        return NULL;
    }
...
#ifdef FONT_HASH_SDBM
static unsigned int hashfunc(const struct key *key) {
...
#endif
#ifdef FONT_HASH_MURMUR2
uint32_t hashfunc(const struct key *key) {
    uint32_t seed = 0xCEED;
    int len = sizeof(struct key);
...
static const char *text_frag_alt_src =
    "uniform sampler2D map_color;\n"
    "in vec2 var_texcoord;\n"
    "in vec4 var_color;\n"
    "out vec4 frag_color;\n"
...
void text_begin(mat4 clip_from_view, mat4 view_from_world, int alt) {
    ...
    if (!text_prog) {
        ...
        text_uni_alt_view_from_world = glGetUniformLocation(text_prog_alt, "view_from_world");
    }
